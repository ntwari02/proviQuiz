...
router.get("/performance", authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const byCategory = await ExamSession.aggregate([
      { $match: matchUser },
      { $unwind: "$answers" },
      {
        $lookup: {
    ...
    const byTopic = await ExamSession.aggregate([
      { $match: matchUser },
      { $unwind: "$answers" },
      {
        $lookup: {
...
router.get("/weak-areas", authMiddleware, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId;
    if (!userId) return res.status(401).json({ message: "Not authenticated" });
    ...
    const mostMissed = await ExamSession.aggregate([
      { $match: matchUser },
      { $unwind: "$answers" },
